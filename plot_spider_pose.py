...
fig = plt.figure()
def generate_spider_frame(angles):
    '''Generate a 3D plot of a spider pose and return the figure object.
...
def animate(i):
    #animate spiders 
    chromosome = gloChromosome
    ax.clear()
    plt.cla()
    frame = generate_spider_frame(chromosome[i])
    #clear title and add new title
    plt.title(f"Generation {i+1}")
    patches.append([frame])
    return patches

def plot_spider_animation(chromosome):
    '''Plot a 3D animation of a spider moving through a sequence of poses.

    Input:
        angle_sequence: 300 list of 1x24 vectors of joint angles in radians'''

    frame_amount = len(chromosome)
    spiders = [plt.plot([], [])[0] for _ in range(frame_amount)]
    global patches 
    patches = spiders
    global gloChromosome
    gloChromosome = chromosome

    anim = animation.FuncAnimation(fig, animate, frames=len(chromosome), interval=100)
    plt.show()